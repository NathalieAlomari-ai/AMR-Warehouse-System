...
public:
    ...
    //
    // OPERATION_MODE_IMUPLUS fuses accelerometer + gyroscope only.
    // The magnetometer is deliberately excluded because the 24V drive motors
    // generate strong magnetic fields that corrupt absolute-heading readings.
    // In IMUPLUS mode the heading is relative (zeroed at power-on), which is
    // handled on the ROS 2 side by robot_localization with imu0_relative: true.
    bool begin() {
        _left.begin();
        _right.begin();
